...
def write_data(ci,module_name,result,artifacts_user,
    ...
    # if str(data_frame['ci'].item()) == ci:
    #     data_frame[module_name] = result
    if data_frame['ci'].str.contains(ci).any():
        data_frame[module_name][data_frame['ci']==ci] = result
        auth=(username,pwd)
        response = requests.put(url, auth=auth, data=data_frame.to_csv(index=False))
        status = module_name+" : status updated for ci :  " + ci
    else:
        status = module_name+" not found and unable to update for ci : "+ ci
    return status
...
def main():
    ...
    try:
        status = write_data(module.params["ci"],
