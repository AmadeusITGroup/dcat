...
def create_entity(cr,awx_job_id,ritm_number,requestor_name,retention_days,requestor_group,pillar,cmdb,connection_string,mit_ticket):
    ...
    try:   
        ...
        idempotent_obj["status"] = "Creating new entity "+str(e)
        metadata_details = None

    if metadata_details:
        idempotent_obj["status"] = "Metadata details found"
        ...
        mit = metadata_details["mit_ticket"]
        ...
        if cmdb_status =='F'and cmdb != 'F':
            ...
            idempotent_obj["status"] = "Metadata details updated"
            ...
            idempotent_obj["mit_ticket"] = mit
            idempotent_obj["cmdb"] = cmdb
            
        
        else:
            idempotent_obj["status"] = "No change in Metadata details"        
        ...
        new_ent = {'PartitionKey':cr,'RowKey':awx_job_id,'ritm_number':ritm_number,'requestor_name':requestor_name,
                    'retention_days':retention_days,'requestor_group': requestor_group,"pillar": pillar,
                    "mit_ticket": mit_ticket,"ritm_creation_date": formatted_date,'cmdb':'F'}
        ...
        idempotent_obj["status"] = "Metadata details created fresh"
...
def run_module():
    ...
    try:
        ...
        create_entity_result = create_entity(cr= module.params["cr"],awx_job_id=module.params["awx_job_id"],
                                             ...
                                             pillar= module.params["pillar"],cmdb= module.params["cmdb"],
                                             connection_string= module.params["connection_string"],mit_ticket= module.params["mit_ticket"])
