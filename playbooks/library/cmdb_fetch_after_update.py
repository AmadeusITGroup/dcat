...
def cmdb_after_update(conn_str,server_name,cmdb_server,user_name,pass_word,instance_id):
    ...
    try:
        ...
        if len(entries) == 1:
            ...
            new_content_3 = str(values[fieldname_3])
            if new_content_1 =="End of Life" and new_content_2 =="Obsolete" and new_content_3 == "None":
                final_msg_cmdb = "CMDB STATUS UPDATED SUCCESSFULLY that is status field to (End Of Life),status reason field to (Obsolete) and set ADMIN IP to null."
            else:
                final_msg_cmdb = "Failed to update CMDB status kindly check for connection issue" + str(result)
            ...
            encoded_query = urllib.parse.quote(query)
            fields = 'Name,RequestId,AssetID,InstanceId,' + fieldname_1  +' , ' + fieldname_2 +' , ' + fieldname_3
            parameters = 'q=' + encoded_query + '&fields=values(' + fields + ')'
            url = entry_url + '/BMC.CORE:BMC_ComputerSystem?' + parameters
            response = requests.get(url=url, headers=headers)
            if JWT_AUTH_TOKEN is not None:
        ...
        final_msg_cmdb = "Exception in cmdb_after_update module.Kindly check if inputs are valid :"+str(err)
        if JWT_AUTH_TOKEN is not None:
            logout_cmdb(cmdb_server=cmdb_server, auth_token=JWT_AUTH_TOKEN,
                          cmdb_object=CMDB_USER_OBJ, conn_str=conn_str)
            JWT_AUTH_TOKEN = None
