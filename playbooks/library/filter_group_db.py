...
def get_owner(conn_str,server_name,cmdb_server,user_name,pass_word,instance_id):
    ...
    # entry_url = "http://" + cmdb_server +":8008/api/arsys/v1/entry"
    ...
    data=result['entries'][0]['values']['SupportedBy']
    if data:
     return data
    else:
      data = "Exception" + str(result)
      return data 
def run_module():
    global JWT_AUTH_TOKEN, CMDB_USER_OBJ
    fields = {
    ...
    returnvalue = {}
    try:
        group_by_owner = get_owner(conn_str=module.params["conn_str"],server_name = module.params["server_name"],cmdb_server= module.params["cmdb_server"],user_name= module.params["user_name"],pass_word= module.params["pass_word"],instance_id= module.params["instance_id"])
