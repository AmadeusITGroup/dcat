        ...
        try:
            ...
            for nvme_storage in disks:
               if "fio" in nvme_storage or "nvm" in nvme_storage:
                    ...
                    cmd4 = "sudo -i dd if=/dev/zero of=/dev/"+nvme_storage+" bs=1M count=1 status=progress"
                    stdin, stdout, stderr = ssh.exec_command(cmd4)
                    stdout.channel.recv_exit_status()
                    time.sleep(2)
                    ssh.close()
                    idempotent_obj.append(nvme_storage+" NVME/FusionIO cleaned")
               else:
                   idempotent_obj.append(nvme_storage+" Not cleaned")
