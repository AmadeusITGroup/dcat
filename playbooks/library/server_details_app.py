...
def cmdb_fetch(conn_str,server_name,cmdb_server,user_name,pass_word,instance_id):
    ...
    ############# checking the status reason part ###################
                    ...
                    if status_of_server == "In Production" or status_of_server == "Deco in Progress":
                        final_msg ="You are good to proceed with deco for server " + server_name + "."
                    else:
                        final_msg = "The server "+server_name+" is not In Production status or Deco in progress status."+" Kindly check with application team"
