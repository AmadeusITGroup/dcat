...
from ansible.module_utils.cmdb.helpers_for_cmdb import login_cmdb,  logout_cmdb, create_table_cmdb_user, fetch_cmdb_users_from_azure_storage, update_cmdb_user_azure
...
def get_owner(conn_str,server_name,cmdb_server,user_name,pass_word,instance_id):
    ...
    if "NO LOGIN" in JWT_AUTH_TOKEN:  # response.status_code != requests.codes.ok:
        data = "An error occurred while fetching owner: " + str(JWT_AUTH_TOKEN)
        JWT_AUTH_TOKEN = None
        return data
    ...
    try:
        ...
        data=result['entries'][0]['values']['OwnedBy']
        if data:
         return data
        else:
          data = "NA"
