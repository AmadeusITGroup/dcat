...
description: This module Queries the RTU SSP application URL to form the target app which will be validated 
on the target host.
...
options:
    host:
        ...
        type: str
    ssp_app:
        description: SSP application server 
        required: true
        type: str
author:
    - Prashanth k (@pk)
...
def fetch_app(host,ssp_app,instance_id):
    url = "https://"+ssp_app+"/rtu/?serverIsVirtual=Yes&rtuSubArea=SSP"
...
def run_module():
    ...
    fields = {
         ...
         "ssp_app":{"required":True, "type":"str"},
         "instance_id":{"required":True, "type":"str"}
    }
    module = AnsibleModule(argument_spec = fields)
    try:
        app_name = fetch_app(module.params["host"],
                             module.params["ssp_app"],
                             module.params["instance_id"])
        os_name = os_type(module.params["host"],module.params["ssp_app"])
        #print("this is executed")
        #returnValue["app_name"] = app_name
        returnvalue["OS_type"] = os_name
        returnvalue["SSP_application_name"] = app_name
