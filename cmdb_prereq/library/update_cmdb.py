...
- Update_CMDB:
            ...
            status_reason_value: 'Obsolete'  #'Obsolete' #'Not Deco in Progress'
...
def cmdb_update(conn_str,server_name,cmdb_server,user_name,pass_word,status_value,status_reason_value,instance_id):
    ...
    try :
        ...
        if len(entries) == 1:
            ...
            body = {'values': {fieldname_1: status_value, fieldname_2: status_reason_value,fieldname_3: new_content_3}}
            ...
            if response.status_code != requests.codes.no_content:
                ...
                final_msg ="Error happened while updating the CMDB status.Kindly check. Status code and request code is different"
            else:
                final_msg = "CMDB STATUS UPDATED SUCCESSFULLY that is status field to (Deployed) , status reason field to (Deco in Progress)."

        elif len(entries)== 0:
            final_msg = "no entries exist for the server"

        else:
            if instance_id =="":
                final_msg = "Duplicate entries exist and no instance ID provided"
            else:
                JWT_AUTH_TOKEN, CMDB_USER_OBJ = login_cmdb(conn_str=conn_str,
                                                           cmdb_server=cmdb_server,
                                                           password=pass_word)
                ...
                if len(entries)== 1:
                    ...
                    final_msg = "Duplicate entries exist but instance id doesnt match"
                else:
                    final_msg = "Duplicate entries exist with same instance id"
